...
class MDudoTrainer:
    ...
    def m_cfr_plus(self,
               ...
               p1: np.ndarray,
               curr_player_n: int,
               gamma: int) -> np.ndarray:  # history -> isClaimed
        ...
        strategy = node.getStrategy(gamma, p0 if player == 0 else p1, player, curr_player_n)
        ...
        for a in range(node.NUM_ACTIONS):
            ...
            if player == 0:
                util[:, :, a] = self.m_cfr_plus(iter_n, nextHistory, p0 * strategy[:, a], p1, curr_player_n, gamma)
                for i in range(self.NUM_SIDES):
                    nodeUtil[i, :] += util[i, :, a] * strategy[i, a]
            else:
                util[:, :, a] = self.m_cfr_plus(iter_n, nextHistory, p0, p1 * strategy[:, a], curr_player_n, gamma)
                for j in range(self.NUM_SIDES):
    ...
    def train(self,
        ...
        for i in range(1, iterations + 1):
            ...
            for player in range(2):
                ...
                util += self.m_cfr_plus(i, startClaims, np.array([1] * 6), np.array([1] * 6), player, max(i - d, 0))
                # TODO: call exploitability counting here for the opponent
            cur_res = np.sum(util / i / 36 / 2)
            if i % 10 == 0:
                # cur_res = np.sum(util / iterations / 36)
                results.append(cur_res)
        utils.save_result_to_file(results, "CFR_plus")
...
if __name__ == '__main__':
    ...
    d = 0
    # w^T = max(T - d; 0)
    TrainRes = MDudoTrainer().train(iterations, d)
